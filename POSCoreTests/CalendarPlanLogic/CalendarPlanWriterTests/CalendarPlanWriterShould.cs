...
namespace POSCoreTests.CalendarPlanLogic.CalendarPlanWriterTests
{
    public class CalendarPlanWriterShould
    {
        ...
        private void AssertTableDates(Table calendarPlan, ConstructionMonth[] totalCalendarWorkConstructionMonths)
        {
            ...
            for (int i = 0; i < totalCalendarWorkConstructionMonths.Length; i++)
            {
                ...
                var monthNames = DateTimeFormatInfo.CurrentInfo.MonthNames.ToList();
        ...
        public void Write_CalendarPlan1Month_SaveCorrectCalendarPlan()
        {
            ...
            var templatePath = @"..\..\..\..\POSCore\CalendarPlanLogic\CalendarPlanTemplates\CalendarPlan1MonthTemplate.docx";

            calendarPlanWriter.Write(preparatoryCalendarPlan, mainCalendarPlan, templatePath);

            using (var document = DocX.Load(Directory.GetCurrentDirectory() + @"\CalendarPlan1Month.docx"))
            {
        ...
        public void Write_CalendarPlan2Month_SaveCorrectCalendarPlan()
        {
            ...
            using (var document = DocX.Load(Directory.GetCurrentDirectory() + @"\CalendarPlan2Month.docx"))
            {
                var preparatoryCalendarPlanTable = document.Tables[0];
                var mainCalendarPlanTable = document.Tables[1];
        ...
        public void Write_CalendarPlan3Month_SaveCorrectCalendarPlan()
        {
            ...
            using (var document = DocX.Load(Directory.GetCurrentDirectory() + @"\CalendarPlan3Month.docx"))
            {
                var preparatoryCalendarPlanTable = document.Tables[0];
                var mainCalendarPlanTable = document.Tables[1];
