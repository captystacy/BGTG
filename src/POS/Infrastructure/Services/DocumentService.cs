...
namespace POS.Infrastructure.Services
{
    public class DocumentService : IDocumentService
    {
        ...
        public void Append(string text, int rowIndex, int paragraphIndex, int fontSize = 12)
        {
            _documents[DocumentIndex].Tables[TableIndex].Rows[rowIndex].Paragraphs[paragraphIndex].Append(text).FontSize(fontSize);
        }

        public void InsertDocument(int baseDocumentIndex,int insertedDocumentIndex)
        {
            _documents[baseDocumentIndex].InsertDocument(_documents[insertedDocumentIndex]);
        }
