...
public abstract class DbContextBase<TContext> : DbContext where TContext : DbContext
{
    ...
    private void DbSaveChanges()
    {
        ...
        foreach (var entry in createdEntries)
        {
            ...
            var userName = entry.Property(nameof(IAuditable.CreatedBy)).CurrentValue == null
                ? DefaultUserName
            ...
            if (createdAt != null)
            {
                entry.Property(nameof(IAuditable.CreatedAt)).CurrentValue = DateTime.Parse(createdAt.ToString()!).Year > 1970 
                    ? ((DateTime)createdAt).ToUniversalTime() 
                    : creationDate;
            }
            else
            {
                entry.Property(nameof(IAuditable.CreatedAt)).CurrentValue = creationDate;
            }

            if (updatedAt != null)
            {
                entry.Property(nameof(IAuditable.UpdatedAt)).CurrentValue = DateTime.Parse(updatedAt.ToString()!).Year > 1970 
                    ? ((DateTime)updatedAt).ToUniversalTime() 
                    : creationDate;
            }
            else
            {
                entry.Property(nameof(IAuditable.UpdatedAt)).CurrentValue = creationDate;
            }
        ...
        foreach (var entry in updatedEntries)
        {
            if (entry.Entity is IAuditable)
            {
                ...
                entry.Property(nameof(IAuditable.UpdatedAt)).CurrentValue = DateTime.Now.ToUniversalTime();
                entry.Property(nameof(IAuditable.UpdatedBy)).CurrentValue = userName;
            }
